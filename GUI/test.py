        ...
        if index.isValid():
            ...
            if child is not None:
                ...
                if action == delete or action == cut:
                    self.ui.processWidget.removeCellWidget(index.row(), index.column())
                    for i in range(0, child.columnspan):
                        self.process.remove([child, index.column()+i, index.row(), i+1])
                    child.deleteLater()
                    self.ui.processWidget.setSpan(index.row(), index.column(), 1, 1)
                ...
                if action == add:
                    ...
                    for i in range(0, newOp.columnspan):
                        self.process.insert(self.insertOperatorIndex(index.column() + i, index.row()),
                                            [newOp, index.column() + i, index.row(), i + 1])
                    self.ui.processWidget.setSpan(index.row(), index.column(), 1, newOp.columnspan)
                    self.ui.processWidget.setCellWidget(index.row(), index.column(), newOp)
                    self.ui.processWidget.resizeRowToContents(index.row())
                    self.showParameterSetting(index.row(), index.column())
